...
import { BarChart3, Bot, LayoutDashboard, LogOut, Users } from "lucide-react";
...
const menuItems = [
  ...
  { icon: Users, label: "Peserta MDP", path: "/peserta" },
  { icon: BarChart3, label: "Laporan", path: "/laporan" },
  { icon: Bot, label: "Asisten AI", path: "/asisten" },
];
        ...
        <div className="flex flex-col items-center gap-8 p-8 max-w-md w-full">
          <div className="flex flex-col items-center gap-3">
            ...
            <h1 className="text-2xl font-bold tracking-tight text-center text-foreground">
              MDP Assessment System
            </h1>
            <p className="text-sm text-muted-foreground text-center max-w-sm">
              Sistem Penilaian Kompetensi Program Pengembangan Manajemen. Silakan masuk untuk melanjutkan.
            </p>
          </div>
          <Button
            onClick={() => { 
      ...
      <div className="relative" ref={sidebarRef}>
        <Sidebar collapsible="icon" className="border-r-0" disableTransition={isResizing}>
          ...
          <SidebarContent className="gap-0 pt-3">
            {!isCollapsed && (
              <p className="px-5 pb-2 text-xs font-semibold text-sidebar-foreground/40 uppercase tracking-wider">
                Menu Utama
              </p>
            )}
          ...
          <SidebarFooter className="p-3 border-t border-sidebar-border">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-3 rounded-xl px-2 py-2 hover:bg-sidebar-accent/50 transition-colors w-full text-left focus:outline-none">
          ...
          <div className="flex border-b h-14 items-center justify-between bg-background/95 px-4 backdrop-blur sticky top-0 z-40">
            <div className="flex items-center gap-3">
              ...
              <span className="font-semibold text-foreground">
                {activeMenuItem?.label ?? "MDP Assessment"}
              </span>
            </div>
          </div>
        )}
